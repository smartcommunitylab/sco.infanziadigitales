...
public class RepositoryManager {
	...
	public List<DiaryKid> updateDiaryKidPersons(String appId, String schoolId) {
		...
		for (KidProfile kp : profiles) {
			...
			if (diaryKid != null) {
				...
				/*
				 * if (kp.getDiaryTeachers() != null) { for (DiaryTeacher dt:
				 * kp.getDiaryTeachers()) { if
				 * (!existingTeachers.contains(dt.getTeacherId())) { Teacher
				 * teacher = getTeacher(dt.getTeacherId(), appId, schoolId);
				 * diaryKid.getPersons().add(teacher.toDiaryKidPerson(true)); }
				 * } }
				 */
				...
				/*
				 * if (kp.getDiaryTeachers() != null) { for (DiaryTeacher dt:
				 * kp.getDiaryTeachers()) { Teacher teacher =
	...
	public List<KidProfile> getKidProfilesByParent(String appId, String username) throws ProfileNotFoundException {
		...
		q = appQuery(appId);
		q.addCriteria(
				new Criteria("persons").elemMatch(new Criteria("personId").is(p.getPersonId()).and("parent").is(true)));
	...
	public List<DiaryKidProfile> getDiaryKidProfilesByAuthId(String appId, String schoolId, String authId,
		...
		q.addCriteria(
				new Criteria("persons").elemMatch(new Criteria("personId").is(authId).and("parent").is(!isTeacher)));
	...
	public KidCalAssenza getAbsence(String appId, String schoolId, String kidId, long date) {
		Query q = kidQuery(appId, schoolId, kidId);
		q.addCriteria(new Criteria().andOperator(new Criteria("dateFrom").lte(date), new Criteria("dateTo").gte(date)));
	...
	public KidConfig saveAbsence(KidCalAssenza absence) {
		...
		q.addCriteria(new Criteria().andOperator(new Criteria("dateFrom").gte(absence.getDateFrom()),
	...
	public List<InternalNote> getInternalNotes(String appId, String schoolId, String[] sectionIds, long date) {
		...
		if (sectionIds != null && sectionIds.length > 0) {
			...
			q = schoolQuery(appId, schoolId);
			q.addCriteria(new Criteria("date").is(timestampToDate(date)));
			q.addCriteria(new Criteria().orOperator(new Criteria("sectionIds").in((Object[]) sectionIds),
			...
			if (conf != null && !conf.getServices().getBus().isActive() || assenza != null) {
				...
				if (stop != null) {
					...
					if (ritiro != null) {
						...
						busKidProfile.setBusStop(conf != null ? conf.getServices().getBus().getDefaultIdBack()
	...
	public List<SectionData> getSections(String appId, String schoolId, Collection<String> sections, long date) {
		...
		for (KidProfile kp : kids) {
			...
			skp.setBus(
					new ServiceProfile(kp.getServices().getBus().isEnabled(), conf != null ? conf.busActive() : true));
	...
	private List<KidProfile> readKidsForSections(String appId, String schoolId, Collection<String> sections) {
		Query q = schoolQuery(appId, schoolId);
		if (sections != null) {
	...
	private Long computeTime(long date, KidConfig kidConfig, KidProfile kp, SchoolProfile profile) {
		...
		String exit = kp.getServices().getPosticipo().isEnabled() ? profile.getPosticipoTiming().getToTime()
	...
	public List<DiaryEntry> getDiary(String appId, String schoolId, String kidId, String search, Integer skip,
			Integer pageSize, Long from, Long to, String tag) {
		Query q = kidQuery(appId, schoolId, kidId);
	 ...
	 * @return
	 */
	public List<MultimediaEntry> getMultimediaEntries(String appId, String schoolId, String kidId, Integer skip,
			Integer pageSize, Long from, Long to) {
	...
	public ChatMessage removeChatMessage(String appId, String schoolId, String messageId) {
		Criteria criteria = new Criteria("appId").is(appId).and("schoolId").is(schoolId).and("messageId").is(messageId);
		Query query = new Query(criteria);
	 ...
	 * @return
	 */
	public Map<String, Map<String, Integer>> getAllUnreadChatMessageCount(String appId, String schoolId,
			String sender) {
		Map<String, Map<String, Integer>> result = new HashMap<String, Map<String, Integer>>();
		Criteria criteria = new Criteria("appId").is(appId).and("schoolId").is(schoolId).and("sender").is(sender)
				.and("seen").is(false);
		Query query = new Query(criteria);
	...
	public List<UsageEntity> findUsageEntities(String appId, String schoolId, UsageAction action, UsageActor from,
	...
	public long countUsageEntities(String appId, String schoolId, UsageAction action, UsageActor from, UsageActor to,
			Object extra, Long fromTime, Long toTime) {
		Criteria criteria = new Criteria("appId").is(appId).and("schoolId").is(schoolId);
		if (action != null) {
			criteria = criteria.and("action").is(action);
		}
